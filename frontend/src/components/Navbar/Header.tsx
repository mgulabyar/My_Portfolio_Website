  ...
  return (
    ...
    >
      ...
      >
        {navLinks.map((item, index) => (
          <NavLink 
            key={index} 
            to={item.path}
            className={({ isActive }) => `text-sm font-medium transition-colors duration-300 relative group py-1 ${
            ...
            {({ isActive }) => (
              <>
                ...
                <span className={`absolute bottom-0 left-0 h-0.5 bg-brand-orange transition-all duration-300 ${
                  isActive ? 'w-full shadow-glow-orange' : 'w-0 group-hover:w-full shadow-glow-orange'
                }`}></span>
              </>
            )}
          </NavLink>
      ...
      >
        <NavLink 
          to="/contact"
          className={({ isActive }) => `relative overflow-hidden px-5 py-2 rounded-lg border border-brand-orange font-bold text-xs tracking-wider uppercase transition-all duration-500 flex items-center justify-center group cursor-pointer hover:shadow-glow-orange ${
      ...
      <div className="relative h-6 w-6 z-50 md:hidden">
        ...
        >
          <AnimatePresence mode="wait">
            {isOpen ? (
              <motion.div key="close" initial={{ rotate: -180, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 180, opacity: 0 }} transition={{ duration: 0.5 }}>
                <X className="h-6 w-6 text-brand-orange" />
              </motion.div>
            ) : (
              <motion.div key="menu" initial={{ rotate: 180, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -180, opacity: 0 }} transition={{ duration: 0.5 }}>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <motion.div variants={listContainerVariants} className="flex flex-col gap-6">
              {[...navLinks, { name: 'Contact', path: '/contact' }].map((item, index) => (
                <motion.div key={index} variants={linkItemVariants}>
                  <NavLink 
                    to={item.path}
                    onClick={() => setIsOpen(false)}
                    className={({ isActive }) => `text-sm font-semibold tracking-widest uppercase transition-colors duration-300 py-2 block border-b border-white/5 hover:text-brand-orange hover:pl-3 ${
