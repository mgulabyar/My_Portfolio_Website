          ...
          {filterItems.map((filter) => (
            <button
              ...
              className={`px-5 py-2 rounded-full text-[11px] font-black uppercase tracking-wider transition-all duration-300 cursor-pointer border ${
                selectedFilter === filter ? 'bg-brand-orange border-brand-orange text-white shadow-glow-orange' : 'bg-transparent border-white/10 text-slate-400 hover:border-brand-orange/40 hover:text-brand-orange'
              }`}
            >
              {filter}
            </button>
        ...
        <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project) => (
              <motion.div key={project._id} layout variants={cardVariants} initial="hidden" whileInView="visible" exit={{ opacity: 0, scale: 0.9 }} className="group rounded-3xl border border-white/5 bg-slate-900/20 hover:border-brand-orange/30 overflow-hidden transition-all duration-500 shadow-2xl flex flex-col">
                <div className="relative aspect-16/10 bg-black/40 flex items-center justify-center p-4 overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-brand-dark/80 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-center gap-4 z-20">
                    <button onClick={() => { setActiveProject(project); setCarouselIndex(0); }} className="px-6 py-3 rounded-xl border border-white/20 text-white font-bold text-[10px] uppercase tracking-widest hover:bg-white hover:text-black transition-all cursor-pointer">Preview</button>
                    <a href={`/project/${project._id}`} target="_blank" className="px-6 py-3 rounded-xl bg-brand-orange text-white font-bold text-[10px] uppercase tracking-widest shadow-glow-orange hover:scale-105 transition-all cursor-pointer flex items-center gap-2">Open Tab <ExternalLink size={14} /></a>
                  </div>
                </div>
            ...
            >
              ...
              <div className="flex flex-col lg:flex-row overflow-y-auto">
                <div className="lg:w-1/2 p-8 md:p-12 order-2 lg:order-1">
                  ...
                  <p className="text-slate-400 leading-relaxed text-sm md:text-base font-light mb-8">{activeProject.description}</p>
                  
                  <div className="pt-8 border-t border-white/5">
                    <p className="text-[10px] font-black uppercase text-slate-500 tracking-[0.2em] mb-4">Skills and deliverables</p>
                    <div className="flex flex-wrap gap-2">
                      {activeProject.technologies.map((tech, idx) => (
                        <span key={idx} className="text-[11px] font-bold text-slate-300 bg-white/5 border border-white/10 px-4 py-2 rounded-full">{tech}</span>
                      ))}
                    </div>
                  </div>
                </div>
                ...
                <div className="lg:w-1/2 bg-black/40 p-6 flex items-center justify-center relative order-1 lg:order-2 border-b lg:border-b-0 lg:border-l border-white/5 min-h-87.5">
                  ...
                  <div className="absolute inset-x-4 flex justify-between">
                    <button onClick={prevImage} className="p-3 rounded-full bg-brand-orange/90 text-white shadow-glow-orange cursor-pointer hover:scale-110 transition-transform"><ChevronLeft size={24}/></button>
                    <button onClick={nextImage} className="p-3 rounded-full bg-brand-orange/90 text-white shadow-glow-orange cursor-pointer hover:scale-110 transition-transform"><ChevronRight size={24}/></button>
                  </div>
                </div>
              </div>
            </motion.div>
